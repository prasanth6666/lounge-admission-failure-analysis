...
The prompt sent to Gemini includes the transaction ID, failure categories, and the details of all failed and inconclusive rule findings. When a deterministic failure is present there is always exactly one finding; when findings are inconclusive there may be several, and Gemini is instructed to address all of them. Gemini is also instructed to provide specific next steps per failure type and respond in JSON only.
